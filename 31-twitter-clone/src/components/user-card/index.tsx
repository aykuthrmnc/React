...
const UserCard = ({ user }: { user: { avatar: string; username: string; fullName: string; verified?: boolean } }) => {
  const [following, setFollowing] = useState(false);

  return (
    <Link
      to={`/${user.username}`}
      className="flex gap-3 px-4 py-3 transition-colors hover:bg-[color:var(--background-third)]"
    >
      <div className="flex flex-1 gap-3">
        <img src={user.avatar} className="h-10 w-10 rounded-full object-cover" alt="" />
        <div className="w-[calc(100%-3.25rem)] text-left">
          <div className="flex items-center truncate font-bold leading-5">
            {user.fullName}{" "}
            {user.verified && (
              <svg
                viewBox="0 0 22 22"
          ...
          <div className="truncate text-[color:var(--color-base-secondary)]">@{user.username}</div>
        </div>
      </div>
      <div>
        {following ? (
